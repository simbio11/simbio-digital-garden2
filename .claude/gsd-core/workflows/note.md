...
- **Global scope**: `C:/Users/cmksc/quartz/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Users/cmksc/quartz/.claude/notes/`)
...
2. Glob `C:/Users/cmksc/quartz/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Users/cmksc/quartz/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Users/cmksc/quartz/.claude/notes/` — works in any directory
