...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 중증 고혈압, 뇌동맥류, 심근경색 등 심혈관계 고위험 환자나 녹내장 환자에게는 흉강 내압 급증에 따른 뇌혈류 변동 및 미주신경 실신 위험이 있으므로 시행 금기, 음성이라도 디스크를 배제할 수 없음(위음성 주의).</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 22% ~ 35% (낮음, 단독 선별 배제 불가)<br>• **특이도 (Specificity)**: 94% ~ 95% (매우 높음, 확진용 가치 탁월)<br>• **양성 우도비 (LR+)**: 3.50 ~ 4.50 |
...
```mermaid
graph TD
    Strain["성문 폐쇄 후 하복부 힘주기<br>(Valsalva Maneuver / 10~15초)"] --> PressInc["복강내압(IAP) & 흉강내압(ITP) 급증<br>(횡격막 및 복벽근 동시 수축)"]
    PressInc --> Batson["하대정맥 압박 $\rightarrow$ 척추내 정맥 역류<br>(Batson 경막외 정맥총 울혈)"]
    Batson --> Thecal["척수강 내 지주막하 압력(CSF Pressure) 급상승<br>(경막낭의 외측 팽창)"]
    Thecal --> CatchCheck{"공간 점유성 병변(디스크/골극/종양) 존재?"}
    CatchCheck -- Yes --> Impinge["팽창된 경막낭과 신경근이 병소와 강하게 충돌<br>⚡ 전형적 국소 요통 및 하지/상지 방사통 재현"]
    CatchCheck -- No --> Normal["정상적인 내압 분산 (통증 없음)"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 안전 착석 (등받이 의자/침상)"] --> Step2["Step 2: 심호흡 후 성문 폐쇄 (입과 코 막기)"]
    Step2 --> Step3["Step 3: 배변하듯 10~15초간 하복부 강한 힘주기"]
    Step3 --> CheckPain{"국소 통증 또는 원위부 방사통 재현?"}
    CheckPain -- 방사통/찌릿함 재현 --> PosResult["🎯 양성 판정 (추간판탈출증/공간점유병변 확진)"]
    CheckPain -- 통증 없음 --> CoughCheck["Step 4: 기침/재채기 유도 (Dejerine 3징후 연계)"]
    CoughCheck --> NegResult["✅ 음성 판정"]
```
