...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 급성 늑골 골절, 복부 탈장, 중증 심혈관 질환 환자 시 과도한 기침/힘주기 주의, 단순 늑간근/요방형근 근육통 환자의 기침 시 국소 통증을 신경근 방사통과 명확히 구별 필요.</font>
...
| **3대 구성 요소** | ① 기침 (Coughing)<br>② 재채기 (Sneezing)<br>③ 배변 시 힘주기 (Straining at stool / [[Valsalva test]]) |
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 25% ~ 35% (음성이라도 디스크 완전 배제 불가)<br>• **특이도 (Specificity)**: 94% ~ 97% (기침·재채기 시 다리 방사통 재현 시 디스크 확진 가치 극대화)<br>• **양성 우도비 (LR+)**: 4.0 ~ 5.5 |
...
```mermaid
graph TD
    TriadAction["3대 복압 증가 유발 동작<br>(기침 / 재채기 / 배변 시 힘주기)"] --> PresSpike["흉강·복강 내압의 급격한 순간 스파이크(Spike)<br>(횡격막 및 복벽 근육군의 강력한 폭발적 수축)"]
    PresSpike --> BatsonReflux["복부 대정맥 압박 $\rightarrow$ 무판막 Batson 정맥총 역류<br>(척추관 내 경막외 정맥망의 급격한 충혈)"]
    BatsonReflux --> Intrathecal["척수강 내 뇌척수액(CSF) 및 경막외압 급상승<br>(척수 경막낭의 외측 팽창)"]
    Intrathecal --> LesionCatch{"척추관 내 점거성 병변(디스크/종양/골극) 존재?"}
    LesionCatch -- Yes --> NervePinch["팽창된 신경근 슬리브가 병소에 순간적으로 강타·압박<br>⚡ 척추 국소 통증 및 말초 방사통(하지/상지) 재현"]
    LesionCatch -- No --> Normal["정상적인 내압 완충 (통증 없음)"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 착석 또는 앙와위 세팅"] --> Step2["Step 2: 1차 배변 힘주기 (Valsalva 동작 시행)"]
    Step2 --> Check1{"허리 통증 및 다리 저림 발생?"}
    Check1 -- Yes --> Positive["🎯 데제린 징후 양성 (추간판탈출증 확진)"]
    Check1 -- No --> Step3["Step 3: 2차 기침 유도 (연속 2~3회 기침)"]
    Step3 --> Check2{"기침 시 척추 울림 및 방사통?"}
    Check2 -- Yes --> Positive
    Check2 -- No --> Step4["Step 4: 3차 재채기 병력 확인 (또는 코 풀기)"]
    Step4 --> Check3{"방사통 호소?"}
    Check3 -- Yes --> Positive
    Check3 -- No --> Negative["✅ 음성 판정"]
```
