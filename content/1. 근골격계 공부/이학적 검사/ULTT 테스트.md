...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 정상인에서도 끝범위 신장 시 연부조직 당김이 발생하므로 반드시 경추 측굴에 따른 증상 변화(구조적 감별, Structural Differentiation)를 확인해야 하며, 급성 신경초 손상이나 심한 불안정성 시 급격한 견인 금기.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: **97%** (ULTT 1 정중신경 기준, 음성 시 디스크 배제에 결정적)<br>• **특이도 (Specificity)**: 22 ~ 75% (단독 확진용으로는 위양성 주의 필요)<br>• **음성 우도비 (LR-)**: 0.12 (Wainner et al., 2003) |
...
```mermaid
graph TD
    Start["환자 앙와위 세팅: 검사자가 환측 견갑대 하강(Depression) 및 상완골 견고히 고정"] --> Sequence["상지 관절 순차 가동 (외전 → 신전/굴곡 → 회외/회내 → 완관절 신전/굴곡)"]
    Sequence --> Tension["해당 말초신경([[정중신경]]/[[요골신경]]/[[척골신경]]) 주행 경로를 따라 축성 인장 장력 극대화"]
    Tension --> DiffRoute{"경추 반대측 측굴(Contralateral Lateral Flexion) 부가"}
    DiffRoute -- "상지 방사통/저림 즉각 악화" --> TruePos["🎯 양성 판정: 진정한 신경성 병변 (경추 신경근병증/신경포착 확진)"]
    DiffRoute -- "증상 변화 없음 (단순 근육 당김)" --> FalsePos["⚠️ 음성 판정: 근육·인대성 연부조직 단축(Tightness)으로 감별"]
    DiffRoute -- "경추 동측 측굴 시 증상 경감" --> Confirm["✅ 신경 긴장도 해소 확인 (신경역동학적 양성 입증)"]
```
...
2. **Elvey, R. L. (1997)**. Physical evaluation of neural tissue in disorders of the neuromusculoskeletal system. *Journal of Hand Therapy*, 10(2), 122-129. [PMID: 9188049](https://pubmed.ncbi.nlm.nih.gov/9188049/)
