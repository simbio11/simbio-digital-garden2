...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 슬괵근(Hamstrings) 단축 시 오금 당김을 방사통으로 오인하는 위양성 주의, 슬관절 굴곡 구축 또는 고관절 중증 관절염 시 검사 제한, 족관절 배굴(Bragard)을 부가하여 신경성 vs 근육성 감별 필수.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 41% ~ 49% (단독 선별 가치는 앙와위 SLR보다 낮음)<br>• **특이도 (Specificity)**: 69% ~ 85% (체간 후방 전도 동반 시 특이도 상승)<br>• **Waddell 주의분산 일치도**: 앙와위와 30° 이상 불일치 시 꾀병 특이도 90% 이상 |
...
graph TD
    ...
    FlipCheck -- 진성 신경근병증 --> FlipSign["⚡ 플립 징후 (Flip Sign) 발생<br>상체가 뒤로 젖혀지며 침상을 지탱"]
...
```mermaid
graph TD
    Step1["Step 1: 환자 침상 걸터앉기 (고관절·슬관절 90° 굴곡)"] --> Step2["Step 2: 자연스러운 주의 분산 유도 (발목/무릎 진찰)"]
    Step2 --> Step3["Step 3: 환측 슬관절 점진적 완전 신전 (0°)"]
    Step3 --> CheckResult{"상체 움직임 및 통증 반응?"}
    CheckResult -- 상체 뒤로 젖혀짐 + 방사통 --> TrueRad["🎯 진성 양성 (Flip Sign (+), 신경근병증)"]
