...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 급성 견관절 탈구 직후 무리한 외회전 가인 금기, 단순 관절낭 구축으로 인한 가동 제한성 통증과 진정한 탈구 불안감(Apprehension)을 명확히 구별.</font>
...
| **타겟 병소 (Target)** | • 고관절: [[대퇴비구순]](Acetabular Labrum: 전방/후방 비구순), 대퇴골두 경부 접합부<br>• 견관절: 전하방 관절와순([[반카르트 병변]]), 하관절상완인대복합체(IGHL) |
| **의심 질환 (Suspected Pathologies)** | [[비구순 파열]], 대퇴비구 충돌증후군(FAI), [[어깨 전방 탈구]], 재발성 관절 불안정성 |
| **진단 정확도 (Diagnostic Accuracy)** | • **고관절 비구순 (FADDIR)**: 민감도 94%~99%, 특이도 25%~43% (선별 가치 우수)<br>• **견관절 전방 불안**: 민감도 65%~72%, 특이도 90%~99% (확진 가치 우수) |
...
```mermaid
graph TD
    subgraph HipLabrum ["고관절 비구순 유발 경로"]
        H1["앙와위 고관절 완전 굴곡(Flexion) + 외전(Abduction) + 외회전(ER) 세팅"] --> H2["점진적으로 신전(Extension) + 내전(Adduction) + 내회전(IR)으로 회전 기동"]
        H2 --> H3["대퇴골두 경부(Head-Neck junction)가 전상방 비구순(Labrum)을 강하게 전단 압박(Shear)"]
        H3 --> H4["⚡ 서혜부 찝힘 통증 및 덜컥거리는 소리(Clunk) 유발<br>🎯 전방 비구순 병변 양성"]
    end
    subgraph ShoulderInstability ["견관절 전방 불안정 유발 경로"]
        S1["견관절 90° 외전(Abduction) + 주관절 90° 굴곡 세팅"] --> S2["수동적 외회전(External Rotation) 점진적 말기 가인"]
        S2 --> S3["상완골두가 전하방으로 미끄러지며 전방 관절낭 및 손상된 관절순을 전방 압박"]
        S3 --> S4["⚡ 환자의 급격한 불안감(Apprehension) 표출 및 반사적 근방어(Guarding)<br>🎯 전방 불안정성 양성"]
    end
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 앙와위, 고관절 굴곡·외회전·외전(FABER 자세) 세팅"] --> Step2["Step 2: 검사자가 하지 지지하며 신전·내회전·내전으로 호를 그리며 가동"]
    Step2 --> Check1{"서혜부 깊은 통증 또는 덜컥음(Clunk) 발생?"}
    Check1 -- Yes --> PosAnterior["⚡ 앞비구순(Anterior Labrum) 병변 양성"]
    Check1 -- No --> Step3["Step 3: 순서 역전 기동 (신전·내전 상태에서 굴곡·외전 유도)"]
    Step3 --> Check2{"둔부 후외측 통증 또는 소리 재현?"}
    Check2 -- Yes --> PosPosterior["⚡ 뒤비구순(Posterior Labrum) 병변 양성"]
    Check2 -- No --> Neg["✅ 음성 판정"]
```
