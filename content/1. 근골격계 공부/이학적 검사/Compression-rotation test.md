...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 상완이두근 장두 건염, 견봉하 충돌증후군과의 통증 부위 감별(어깨 심부 통증 vs 전방 표재성 건 통증) 필수.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 24% ~ 40% (위음성 가능성 있어 타 검사 병용 필요)<br>• **특이도 (Specificity)**: 76% ~ 90% (관절 내 기계적 증상 확진력 양호)<br>• **양성 우도비 (LR+)**: 1.5 ~ 3.6 |
...
```mermaid
graph TD
    Position["앙와위 견관절 90° 외전 & 주관절 90° 굴곡 위치"] --> AxialLoad["검사자가 주관절을 통해 상완골 장축 방향으로<br>강력한 수직 축성 압박력(Axial Compression) 가인"]
    AxialLoad --> Sandwich["상완골두(Humeral Head)가 관절와(Glenoid Fossa)에 밀착되며<br>상부 관절와순(Superior Labrum)을 샌드위치처럼 압박"]
    Sandwich --> GrindRotation["압박을 유지한 채 상완골을 내회전 및 외회전<br>(Grinding & Shearing Stress 가인)"]
    GrindRotation --> LabralCatch["파열되거나 헐거워진 관절와순 조각이 골두 사이에 끼임<br>(Mechanical Catching / Subluxation)"]
    LabralCatch --> Provocation["⚡ 관절 깊은 곳의 덜컥거림(Clicking) 및 예리한 통증 유발<br>🎯 양성 판정: SLAP 병변 강력 시사"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 앙와위 착석, 어깨 90° 외전 & 팔꿈치 90° 굴곡 세팅"] --> Step2["Step 2: 검사자가 환자 팔꿈치를 감싸 쥐고 상완골 축성 압박"]
    Step2 --> Step3["Step 3: 압박을 유지한 채 전완을 축으로 어깨 내회전 및 외회전 가동"]
    Step3 --> Check{"어깨 관절 심부의 덜컥음(Click) 또는 걸림(Catching) 발생?"}
    Check -- Yes --> Positive["⚡ 압박 회전 검사 양성 판정 (SLAP 병변 의심)"]
    Check -- No --> Negative["✅ 음성 판정 (오브라이언/스피드 검사 병행)"]
```
