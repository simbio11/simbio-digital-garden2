...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 검사 중 심한 어지럼증이나 실신 징후 발생 시 즉시 중립위로 복귀, 음성이라도 VBI를 완전히 배제할 수 없으므로 추나 시 환자 눈동자와 안색을 상시 모니터링.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 0% ~ 20% (음성이라도 VBI 완전 배제 불가)<br>• **특이도 (Specificity)**: 85% ~ 95% (증상 유발 시 혈류 장애 확진)<br>• **양성 우도비 (LR+)**: 3.5 ~ 8.0 |
...
```mermaid
graph TD
    NeckManeuver["경추 최대 신전(Extension) + 편측 최대 외회전(Rotation) 유도"] --> StretchArtery["회전 반대측(대측) C1-C2 부위 [[추골동맥]]이 환추 후궁에 의해 신장 및 압박"]
    StretchArtery --> Stenosis["대측 추골동맥 혈류의 80~90% 순간적 협착 발생"]
    Stenosis --> Compensation{"회전 동측 추골동맥의 보상적 뇌간 혈류 공급 상태"}
    Compensation -- "동측 동맥 정상 개방" --> Normal["후방 순환 혈류 유지 -> 무증상 (음성)"]
    Compensation -- "동측 동맥 죽상경화/협착/저형성증" --> Ischemia["⚡ 뇌간(Brainstem) 및 소뇌(Cerebellum) 급성 허혈 발현"]
    Ischemia --> Symptoms["🎯 5D 3N 징후 (어지럼증, 안구진탕, 구역, 복시, 실신) -> 양성 판정"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 앙와위 눕힘 (머리를 침대 끝에 위치)"] --> Step2["Step 2: 양손으로 후두부를 받치고 경추를 부드럽게 신전"]
    Step2 --> Step3["Step 3: 신전 상태를 유지하며 목을 편측으로 45° 이상 최대 외회전"]
    Step3 --> Step4["Step 4: 해당 자세를 20~30초간 유지하며 환자 눈동자/표정/대화 모니터링"]
    Step4 --> Check{"어지럼증, 안구진탕, 구역, 발음 곤란 등이 나타나는가?"}
    Check -- Yes --> Pos["⚡ 양성 판정 (즉시 머리 중립위 복귀! 경추 회전 추나 절대 금기)"]
    Check -- No --> Step5["Step 5: 중립 10초 휴식 후 반대쪽 동일하게 검사"]
```
...
| :--- | :--- | :--- |
| • **Dizziness** (현훈 / 회전성 어지럼증)<br>• **Diplopia** (복시 / 물체가 둘로 보임)<br>• **Dysarthria** (구음장애 / 발음 어눌함)<br>• **Dysphagia** (연하곤란 / 침 삼키기 어려움)<br>• **Drop attacks** (낙상 발작 / 의식소실 없는 급성 쓰러짐) | • **Nystagmus** (안구진탕 / 눈동자 떨림)<br>• **Nausea** (구역 및 구토감)<br>• **Numbness** (안면부 또는 편측 감각 저하/저림) | • Ataxia (보행 실조, 비틀거림)<br>• Tinnitus (이명, 귀울림)<br>• Visual disturbance (시야 흐림, 암점)<br>• Syncope (미주신경성 실신) |
...
2. **Mitchell, J. (2007)**. *Vertebral artery blood flow velocity changes associated with cervical spine rotation: a meta-analysis of the evidence with implications for professional practice*. **Journal of Manual & Manipulative Therapy**, 15(3), 165-175. [PMID: 20046565 (⚠️ 제목 근사 — 확인 필요/2026-09-24)](https://pubmed.ncbi.nlm.nih.gov/19066664/)
...
<!-- 보관 태그(1회용·링크오류, 필요시 복원): 추골뇌저동맥부전, 추나안전, 도수치료선별 -->
