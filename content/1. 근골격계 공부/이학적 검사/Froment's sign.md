...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 엄지 중수지절관절(MCP 관절)의 과신전이 동반되는 잔느 징후(Jeanne's sign) 감별 및 정중신경 지배 [[장무지굴근]](FPL)의 대리 보상 기전 확인 필수.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 88% ~ 94% (척골신경 운동 지배 결손 선별력 탁월)<br>• **특이도 (Specificity)**: 90% ~ 96% (수부 내재근 마비 확진 가치 우수)<br>• **양성 우도비 (LR+)**: 8.8 ~ 15.0 |
...
```mermaid
graph TD
    Normal["정상인: 엄지-검지 측면 파지<br>([[무지내전근]] 강력 수축으로 IP 관절 신전 상태 유지)"]
    UlnarPalsy["[[척골신경]] 손상 / 마비 발생"] --> APWeak["[[무지내전근]](Adductor Pollicis) 완전 마비 또는 위약"]
    APWeak --> PaperTraction["검사자가 물고 있는 종이를 강하게 견인"]
    PaperTraction --> PinchFail["엄지 측면 압박력(Pinch power) 급격히 저하"]
    PinchFail --> Compensate["보상 기전 가동: [[정중신경]] 지배 하의<br>[[장무지굴근]](FPL)을 과도하게 대리 수축"]
    Compensate --> IPFlexion["⚡ 엄지 지절간관절([[IP 관절]])이 갈고리처럼 급격히 굴곡됨<br>🎯 양성 판정: 프로망 징후 확진"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 착석, 종이(또는 명함) 준비"] --> Step2["Step 2: 양손 엄지와 검지 측면으로 종이 맞잡기"]
    Step2 --> Step3["Step 3: 환자에게 종이를 놓치지 말도록 지시"]
    Step3 --> Step4["Step 4: 검사자가 종이를 수평 방향으로 강하게 당김"]
    Step4 --> Check{"환측 엄지 IP 관절이 급격히 굽어지는가?"}
    Check -- Yes --> Positive["⚡ 프로망 징후 양성 (척골신경 마비 확진)"]
    Check -- No --> Negative["✅ 음성 (IP 관절 신전 유지하며 정상 저항)"]
```
