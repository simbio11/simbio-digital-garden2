...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 요골신경의 심지 포착인 [[요골관 증후군]](Radial Tunnel Syndrome)이나 후외측 회전 불안정성(PLRI), C6 [[경추 신경근병증]]과의 감별 필수.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 84% ~ 91% (외측 팔꿈치 통증 선별력 우수)<br>• **특이도 (Specificity)**: 50% ~ 88% (타 질환 배제력 보조)<br>• **양성 우도비 (LR+)**: 1.7 ~ 7.5 |
...
```mermaid
graph TD
    Position["주관절 완전 신전 & 전완 회내 세팅<br>([[단요측수근신근]] 기시부 예비 장력 증가)"] --> Palpate["검사자의 엄지로 상완골 [[외측상과]] 압박 고정"]
    Palpate --> ResistExt["환자 주먹 쥐고 손목 능동 신전 & 요사위 가동<br>vs 검사자 굴곡 방향 강한 등척성 저항"]
    ResistExt --> TensileLoad["총신근건([[단요측수근신근]]) 기시부 미세 파열부에<br>강력한 수축성 인장력(Tensile Stress) 집중"]
    TensileLoad --> PeriostealTraction["골-건 이행부(Enthesis) 및 골막 견인 자극<br>혈관신생성 유해수용기(Nociceptors) 기계적 흥분"]
    PeriostealTraction --> SharpPain["⚡ 외측상과 국소부의 날카로운 통증 재현<br>🎯 양성 판정: 외측상과염 확진"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 좌위, 주관절 신전 및 전완 회내 위치"] --> Step2["Step 2: 검사자 엄지로 [[외측상과]] 촉진 및 지지 고정"]
    Step2 --> Step3["Step 3: 환자 주먹 쥐고 수근 신전 및 요측 편위 유도"]
    Step3 --> Step4["Step 4: 검사자가 손등에 굴곡 저항 부하 가인"]
    Step4 --> Check{"외측상과 부착부 통증 유발?"}
    Check -- Yes --> Positive["⚡ 테니스 엘보 검사 양성 판정"]
    Check -- No --> Negative["✅ 음성 판정 (밀 검사 등 추가 확인)"]
```
