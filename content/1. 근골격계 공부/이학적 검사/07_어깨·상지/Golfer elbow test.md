...
> - <font color="#fa7e7e">⚠️ 위양성/위음성 요인 & 검사 금기: 내측상과 바로 뒤 주관(Cubital Tunnel)을 주행하는 [[척골신경]]의 압박성 신경병증([[주관 증후군]]) 및 내측측부인대(MCL) 파열과의 정밀 감별 필요.</font>
...
| **진단 정확도 (Diagnostic Accuracy)** | • **민감도 (Sensitivity)**: 82% ~ 86% (내측 팔꿈치 통증 선별)<br>• **특이도 (Specificity)**: 88% ~ 90% (내측상과염 병변 확진 가치 우수)<br>• **양성 우도비 (LR+)**: 6.8 ~ 8.6 |
...
```mermaid
graph TD
    Position["주관절 신전 & 전완 회외 세팅<br>([[원회내근]], [[요측수근굴근]] 기시부 신장 긴장)"] --> Palpate["검사자의 손으로 상완골 [[내측상과]] 촉진 및 지지 고정"]
    Palpate --> ResistFlex["환자 수근 굴곡 가동 vs 검사자 신전 방향 저항<br>(또는 주관절·수근 완전 수동 신장)"]
    ResistFlex --> TensileLoad["총굴근건 기시부 골-건 접합부(Enthesis)에<br>강력한 수축성·신장성 견인력(Tensile Stress) 집중"]
    TensileLoad --> LocalTraction["변성된 건 조직 내 미세 파열부 자극<br>신생 신경섬유 및 자유 신경종말 기계적 탈분극"]
    LocalTraction --> MedialPain["⚡ 상완골 내측상과 국소부의 날카로운 통증 재현<br>🎯 양성 판정: 내측상과염 확진"]
```
...
```mermaid
graph TD
    Step1["Step 1: 환자 착석, 주관절 신전 및 전완 회외 위치"] --> Step2["Step 2: 검사자 손으로 [[내측상과]] 촉진 및 지지"]
    Step2 --> Step3["Step 3: 환자 수근 능동 굴곡 vs 검사자 신전 저항 가인"]
    Step3 --> Step4["Step 4: 수동 신장 부하 병행 (수근/수지 완전 신전)"]
    Step4 --> Check{"내측상과 부착부 통증 유발?"}
    Check -- Yes --> Positive["⚡ 골퍼 엘보 검사 양성 판정"]
    Check -- No --> Negative["✅ 음성 판정 (척골신경 및 인대 확인)"]
```
...
<!-- 보관 태그(1회용·링크오류, 필요시 복원): 주관절/내측상과 -->
