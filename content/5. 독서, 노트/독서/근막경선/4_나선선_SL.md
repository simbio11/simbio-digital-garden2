...
```mermaid
flowchart TD
    subgraph LOOP1 ["1. 상부 체간 루프 (Upper Trunk Loop)"]
        A["후두골 상항선 / 유양돌기"] --> B["[[두판상근]] & [[경판상근]] (Splenius)"]
        B --> C["C6-T4 극돌기"]
        C --> D["반대측 [[능형근]] ([[대능형근]], [[소능형근]])"]
        D --> E["견갑골 내측연"]
        E --> F["[[전거근]] (Serratus Anterior)"]
        F --> G["제5~9 늑골 외측면"]
        G --> H["[[외복사근]] (External Oblique)"]
        H --> I["복부 백선 & 반대측 [[내복사근]]"]
        I --> J["동측 전상장골극 (ASIS)"]
    end

    subgraph LOOP2 ["2. 하지 등자 루프 (Lower Extremity Loop)"]
        J --> K["[[대퇴근막장근]] (TFL) & [[장경인대]] (ITB)"]
        K --> L["경골 외측과 (Gerdy 결절)"]
        L --> M["[[전경골근]] (Tibialis Anterior)"]
        M --> N["제1중족골 저측면 (발바닥 등자, Stirrup)"]
        N --> O["[[장비골근]] (Peroneus Longus)"]
        O --> P["비골두 (Fibular Head)"]
        P --> Q["[[대퇴이두근 장두]] (Biceps Femoris)"]
        Q --> R["좌골결절 (Ischial Tuberosity)"]
    end

    subgraph LOOP3 ["3. 척추 상행 루프 (Spinal Ascending Loop)"]
        R --> S["천결절인대 & 천골"]
        S --> T["[[척추기립근]] (Erector Spinae)"]
        T --> U["후두골 상항선 (종착 및 시작점 귀환)"]
    end
```
...
| :--- | :--- | :--- |
| **정거장 1** | **후두골 능선 및 유양돌기**<br>(Occipital ridge / Mastoid) | 두경부 기시 닻 |
| **트랙 1** | [[두판상근]] 및 [[경판상근]]<br>(Splenius capitis & cervicis) | 경추 극돌기를 향해 사하방 주행 |
| **정거장 2** | **경추 6번 ~ 흉추 4번 극돌기** | 판상근과 능형근의 척추 극돌기 교차 닻 |
| 트랙 2 | 반대측 [[능형근]]([[대능형근]], [[소능형근]]) $\rightarrow$ 견갑골 내측연 $\rightarrow$ [[전거근]]<br>(Rhomboids $\rightarrow$ Serratus anterior) | 견갑골을 흉벽에 밀착시키는 핵심 근막 슬링(Sling) |
| **정거장 3** | **제5~9 늑골 외측면** | 전거근과 외복사근의 맞물림(Interdigitation) 닻 |
| **트랙 3** | [[외복사근]] $\rightarrow$ 백선 $\rightarrow$ 반대측 [[내복사근]]<br>(External oblique $\rightarrow$ Linea alba $\rightarrow$ Internal oblique) | 복벽을 대각선으로 가로지르는 X자 교차 트랙 |
...
| 트랙 8 | 천결절인대 $\rightarrow$ [[척추기립근]]<br>(Sacrotuberous ligament $\rightarrow$ Erector spinae) | 척추 전장을 따라 후두골로 직행 상행 |
