...
```mermaid
graph TD
    A[스파이로메트리 검사] --> B{FEV1 / FVC < 0.70?}
    B -- Yes --> C[폐쇄성 환기 장애 Obstructive]
    C --> D{기관지확장제 투여 후<br/>ΔFEV1 ≥ 12% AND ≥ 200 mL?}
    D -- Yes --> E[가역성 양성: 천식 Asthma 시사]
    D -- No --> F[비가역성: COPD 또는 만성 기도개형]
    B -- No --> G{FVC < 80% 예측치?}
    G -- Yes --> H[제한성 장애 의심 Suspicious Restrictive]
    H --> I[전신 체적변동기록기로 TLC 측정]
    I --> J{TLC < 80% 예측치?}
    J -- Yes --> K[진성 제한성 장애 확진: IPF, 신경근육질환]
    J -- No --> L[정상 변이 또는 공기포획 동반 폐쇄]
    G -- No --> M[정상 환기 기능 Normal]
```
> ⚠️ **Clinical Pearl**:
...
```mermaid
graph LR
    A[DLCO 이상 판독] --> B[DLCO 감소 < 80%]
    A --> C[DLCO 증가 > 120%]
    B --> D[폐쇄성 동반: 폐기종 Emphysema, LAM]
    B --> E[제한성 동반: 특발성 폐섬유증 IPF, 사코이드증]
    B --> F[스파이로메트리 정상: 폐동맥고혈압 PAH, 만성혈전색전증 CTEPH, 빈혈]
    C --> G[폐포 출혈: Goodpasture 증후군, GPA]
    C --> H[적혈구 증가증, 비만, 좌우단락, 천식 급성기]
```
...
```mermaid
graph TD
    SPL[2차 폐소엽 Secondary Pulmonary Lobule: 1~2.5 cm]
    SPL --> Core[소엽 중심부 Core]
    SPL --> Septum[소엽간 중격 Interlobular Septum]
    Core --> CoreA[소엽 세기관지 Bronchiole]
    Core --> CoreB[소엽 폐소동맥 Pulmonary Arteriole]
    Septum --> SeptA[폐정맥가지 Pulmonary Vein]
    Septum --> SeptB[림프관 Lymphatics]
```
> ⚠️ **Clinical Pearl**:
...
```mermaid
graph TD
    subgraph Alveolar_Pattern["폐포성 질환 (Alveolar Filling)"]
        A1[원인: 수분, 고름, 혈액, 단백질] --> A2[경계가 불분명하고 솜뭉치처럼 융합 Coalescence]
        A2 --> A3[공기 기관지상 Air Bronchogram 양성]
        A3 --> A4[진행 및 소실 속도가 빠름: 수시간~수일]
        A4 --> A5[양측 폐문부 나비날개 음영 Bat-wing: 급성 폐부종]
    end
    subgraph Interstitial_Pattern["간질성 질환 (Interstitial Infiltrate)"]
        B1[원인: 염증 세포 침윤, 섬유화, 암세포] --> B2[경계가 매우 선명하고 융합되지 않음]
        B2 --> B3[공기 기관지상 없음]
        B3 --> B4[진행 및 소실 속도가 매우 느림: 수주~수개월]
        B4 --> B5[망상 Reticular, 벌집폐 Honeycombing, 결절형]
    end
```
...
```mermaid
graph TD
    Nodule[단일 고형 결절 Solid Nodule] --> S1{크기 < 6 mm}
    Nodule --> S2{크기 6 ~ 8 mm}
    Nodule --> S3{크기 > 8 mm}
    S1 --> Low1[저위험군: 추적 불필요]
    S1 --> High1[고위험군: 12개월 후 CT 선택적 고려]
    S2 --> Follow1[6~12개월 후 CT 추적, 필요시 18~24개월 재확인]
    S3 --> HighRisk[3개월 후 CT, PET-CT 검사, 또는 즉시 조직 생검/수술]
```
...
```mermaid
graph TD
    subgraph Carlens["경부 종격동경 (Cervical Mediastinoscopy / Carlens)"]
        C1[경부 흉골상절흔 2 cm 횡절개] --> C2[기관 전방 근막 박리 후 Carina 도달]
        C2 --> C3[생검 가능 구역: 2R, 2L, 4R, 4L, 7번]
        C3 --> C4[접근 불가: 5번 AP window, 6번 전혈관]
    end
    subgraph Chamberlain["전종격동 절개술 (Anterior Mediastinotomy / Chamberlain)"]
        D1[좌측 제2 또는 제3 늑연골 부위 흉골연 절개] --> D2[늑연골 부분 절제 후 전종격동 진입]
        D2 --> D3[생검 목표: 5번 대동맥폐창, 6번 전혈관 림프절]
        D3 --> D4[좌상엽 폐암의 림프절 병기 설정 필수 술기]
    end
```
