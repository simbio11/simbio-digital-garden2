...
section: "Section 4: Diseases and Pathology (Part 1: Congenital, Laryngeal & Obstructive Airway Diseases)"
day: 4
section_pages: "pp. 110-155"
total_plates: 26
...
```mermaid
graph TD
    A[늑연골의 비정상적 과성장] --> B{성장 방향}
    B -- 후방 함입 --> C[오목가슴 Pectus Excavatum]
    B -- 전방 돌출 --> D[새가슴 Pectus Carinatum]
    C --> E[흉부 CT 할러 지수 Haller Index 평가]
    E --> F{Haller Index ≥ 3.25?}
    F -- Yes: 중증 심폐 압박 --> G[최소침습 너스 수술 Nuss Bar 거치]
    F -- No: 경증 --> H[보존적 관찰 및 운동 요법]
    D --> I[골격 유연성 평가]
    I --> J[맞춤형 흉벽 외장 압박기 착용]
```
...
```mermaid
graph TD
    A[척추후측만증 Cobb Angle > 60°] --> B[흉벽 순응도 급감 & 호흡일량 폭증]
    B --> C[얕고 빠른 호흡 & 사강 환기 증가]
    C --> D[폐포 저환기: 저산소증 + 고탄산혈증]
    D --> E[저산소성 폐혈관수축 HPV & 혈관상 소실]
    E --> F[폐동맥고혈압 Pulmonary HTN]
    F --> G[우심실 비대 및 우심부전: 폐성심 Cor Pulmonale]
```
...
```mermaid
graph TD
    A[흡입 항원 Allergen] --> B[기도 상피세포 TSLP / IL-33 분비]
    B --> C[수지상세포 & Th2 세포 / ILC2 분화]
    C --> D[IL-4: B세포 IgE 항체 생산 촉진]
    C --> E[IL-5: 호산구 Eosinophil 증식 및 활성화]
    C --> F[IL-13: 배상세포 점액 과분비 & 기도 평활근 과민성]
    D --> G[비만세포 감작 & 히스타민/류코트리엔 즉각 분비]
    E --> H[호산구 세포독성 단백질 MBP/ECP 상피 탈락]
    G & H & F --> I[기도 부종, 기관지 연축, 점액마개 형성]
    I --> J[만성 기도 개형 Remodeling: 기저막 비후 & 평활근 비대]
```
...
```mermaid
graph TD
    A[담배 연기 Cigarette Smoke] --> B[폐포 대식세포 Macrophage 활성화]
    A --> C[활성산소종 ROS 발생]
    B --> D[CXCL8 & LTB4 분비]
    D --> E[호중구 Neutrophil 대량 유입]
    E --> F[호중구 엘라스타제 분비]
    B --> G[MMP-9 / MMP-12 분비]
    C --> H[알파-1 항트립신 메티오닌 산화 불활성화]
    F & G --> I[폐포 엘라스틴 및 기질 과다 분해]
    H -. 억제 실패 .-> I
    I --> J[폐포벽 파괴: 폐기종 Emphysema 완성]
```
...
```mermaid
graph TD
    subgraph Airway_Epithelium["기도 상피세포"]
        A1[CFTR 결함] --> A2[Cl- 분비 차단]
        A1 --> A3[ENaC 탈억제: Na+ 및 수분 과다 재흡수]
        A2 & A3 --> A4[기도 표면 액체층 ASL 고갈 및 탈수]
        A4 --> A5[점조한 농축 점액 형성 & 섬모 청소 정지]
        A5 --> A6[만성 녹농균 감염 & 기관지확장증]
    end
    subgraph Sweat_Gland["땀샘 도관 상피세포"]
        B1[CFTR 결함] --> B2[관강 내 Cl- 재흡수 불가]
        B2 --> B3[전기적 중성을 위해 Na+ 재흡수 차단]
        B3 --> B4[피부 표면으로 고농도 염분 배출]
        B4 --> B5[땀 전해질 검사 Sweat Chloride ≥ 60 mEq/L: 확진]
    end
```
