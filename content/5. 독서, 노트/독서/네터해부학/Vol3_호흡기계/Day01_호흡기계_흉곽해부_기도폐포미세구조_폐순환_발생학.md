...
```mermaid
graph TD
    A[임신 4-5주: 기관식도격막 융합 부전] --> B[선천성 기관식도루 TEF / 식도폐쇄증 EA]
    B --> C[Type C: 85% 최빈도<br>상부 식도 맹단 폐쇄 + 하부 식도와 기관 분기부 사이 누공]
    C --> D[태아기: 양수 연하 불가 -> 양수과다증 Polyhydramnios]
    C --> E[출생 직후 첫 수유 시: 기침, 사레, 청색증 Choking, Cyanosis]
    C --> F[위장관 공기 유입 -> 복부 팽만]
    C --> G[위액 역류 -> 치명적 화학성 흡인 폐렴]
    C --> H[동반 기형: VACTERL 연관 확인 필수]
```
