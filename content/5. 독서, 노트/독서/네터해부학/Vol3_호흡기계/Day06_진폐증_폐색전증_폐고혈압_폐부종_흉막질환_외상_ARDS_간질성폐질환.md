...
section: "Section 4: Diseases and Pathology (Part 3: Environmental, Vascular, Pleural, Trauma, ARDS & ILD)"
day: 6
section_pages: "pp. 214-290"
total_plates: 23
...
```mermaid
graph TD
    A[결정형 실리카 SiO2 분진 흡입: 1-5 μm] --> B[폐포 대식세포 탐식]
    B --> C[파고리소좀 막 손상 & 대식세포 자멸사]
    C --> D[IL-1β, TNF-α, TGF-β 대량 분비 & 실리카 재유출]
    D --> E[섬유아세포 활성화: 콜라겐 동심원상 침착]
    E --> F[동심원상 초자화 규폐 결절 형성: 폐 상엽 호발]
    F --> G[달걀껍데기 석회화 Eggshell Calcification]
    C --> H[대식세포 살균능 마비: 결핵균 이환율 30배 폭증 Silicotuberculosis]
```
...
```mermaid
graph TD
    A[하지 심부정맥혈전증 DVT 이탈] --> B[폐동맥 혈관 폐색: 폐색전증 PE]
    B --> C[환기는 되나 혈류 차단: 폐포 사강 Dead Space 폭증]
    B --> D[신경반사성 기관지 연축 & 계면활성제 고갈 무기폐]
    C & D --> E[심한 저산소혈증 PaO2 급감 & 호흡곤란]
    B --> F[폐혈관 저항 PVR 급상승]
    F --> G[우심실 급성 확장 및 심실중격 좌측 편위]
    G --> H[좌심실 충만 장애 & 심박출량 급락]
    H --> I[폐쇄성 쇼크 Obstructive Shock 및 급사]
```
...
```mermaid
graph TD
    subgraph Endothelin_Path["1. 엔도텔린 경로 (수축/증식 억제)"]
        A1[엔도텔린-1 ET-1] -->|ETA/ETB 수용체| A2[혈관 수축 & 내막 증식]
        A3[ERA: Bosentan, Macitentan] -. 차단 .-> A2
    end
    subgraph NO_Path["2. 산화질소 cGMP 경로 (이완 촉진)"]
        B1[NO] --> B2[GTP -> cGMP] --> B3[혈관 평활근 이완]
        B4[PDE-5 효소] -->|cGMP 분해| B2
        B5[PDE-5 억제제: Sildenafil] -. 억제 .-> B4
    end
    subgraph Prostacyclin_Path["3. 프로스타사이클린 경로 (확장 촉진)"]
        C1[PGI2 수용체] --> C2[ATP -> cAMP] --> C3[혈관 확장 & 증식 억제]
        C4[합성 PGI2: Epoprostenol, Selexipag] -->|직접 자극| C1
    end
```
...
```mermaid
graph TD
    A[진단적 흉강천자 Thoracentesis 시행] --> B{Light's Criteria 3대 항목 검사}
    B --> C["1. 흉수/혈청 단백질 비 > 0.5<br/>2. 흉수/혈청 LDH 비 > 0.6<br/>3. 흉수 LDH > 혈청 정상상한치 2/3"]
    C -- 3개 중 단 1개라도 양성 --> D[삼출액 Exudate 확진]
    C -- 3개 모두 음성 --> E[누출액 Transudate 확진]
    E --> F[기저 질환 치료: 심부전, 간경변증, 신증후군]
    D --> G[추가 흉수 감별 검사: pH, 포도당, 그람염색/배양, 세포진검사, ADA]
```
...
```mermaid
graph TD
    A[폐 실질 또는 흉벽 손상] --> B[일방향 체크밸브 메커니즘 형성]
    B --> C[흡기 시 공기 유입 / 호기 시 배출 불가]
    C --> D[흉강 내압 대기압 초과 양압 폭증]
    D --> E[동측 폐 완전 허탈 & 기관/종격동 반대측 편위]
    D & E --> F[상대정맥 및 하대정맥 기계적 비틀림/압박]
    F --> G[우심방 정맥 환류량 Venous Return 급감]
    G --> H[폐쇄성 쇼크 Obstructive Shock & PEA 심정지]
    I[침 감압술 Needle Thoracostomy: 14G 바늘 즉각 천자] -. 양압 배출 .-> D
```
