...
section: "Section 5: Therapies and Therapeutic Procedures"
day: 7
section_pages: "pp. 293-344"
total_plates: 27
...
```mermaid
graph TD
    A[β2-작용제 Albuterol / Formoterol] --> B[기관지 평활근 β2 수용체 결합]
    B --> C[Gs 단백질 활성화 & Adenylyl Cyclase 촉진]
    C --> D[세포 내 ATP -> cAMP 증가]
    D --> E[PKA 단백질 인산화효소 A 활성화]
    E --> F[K+ 통로 개방 과분극 & Ca2+ 유입 차단]
    E --> G[소포체 SERCA 펌프 촉진: 세포질 Ca2+ 격리]
    E --> H[MLCK 미오신 경쇄 키나아제 인산화 불활성화]
    F & G & H --> I[기관지 평활근 즉각적 이완 Bronchodilation]
```
...
```mermaid
graph TD
    A[코르티코스테로이드 세포막 통과] --> B[세포질 내 GRα 수용체 결합 & Hsp90 해리]
    B --> C[스테로이드-GR 복합체 핵내 이동]
    C --> D[전사인자 NF-κB / AP-1 직접 결합 차단]
    C --> E[HDAC2 효소 모집: 염색질 응축 탈아세틸화]
    D & E --> F[염증성 사이토카인 IL-4/5/13, 케모카인, COX-2 전사 차단]
    C --> G[DNA GRE 결합: IκBα 및 β2 수용체 합성 촉진]
```
...
```mermaid
graph LR
    subgraph Mallampati_Class["말람파티 구강 시야 분류"]
        A["Class I: 연구개, 구협궁, 구개수, 편도와 전체 노출 (용이)"]
        B["Class II: 연구개, 구협궁, 구개수 상부 노출"]
        C["Class III: 연구개, 구개수 기저부만 노출"]
        D["Class IV: 경구개 Hard Palate만 노출 (극도의 곤란)"]
    end
```
...
```mermaid
graph LR
    subgraph Patient_Thorax["환자 흉강"]
        P[흉관 Tube Thoracostomy]
    end
    subgraph Chamber_1["1. 수집 챔버 Collection"]
        C1[흉수 및 혈액 배액 침전]
    end
    subgraph Chamber_2["2. 물봉 챔버 Water-Seal: 2 cmH2O"]
        C2[일방향 밸브: 역류 차단<br/>호흡 파동 Tidaling 관찰<br/>공기 누출 Air Leak 거품 감시]
    end
    subgraph Chamber_3["3. 흡인 조절 챔버 Suction Control"]
        C3[물기둥 높이: -20 cmH2O 정밀 음압 조절]
    end
    P --> Chamber_1 --> Chamber_2 --> Chamber_3 --> W[벽 흡인기 Wall Suction]
```
