...
graph LR
    subgraph "hCG 분비 곡선"
        A[수정 10일 후 분비 개시] --> B[임신 8~10주 피크: 100,000 mIU/mL]
        B --> C[임신 16~20주 급감: 10,000~20,000 mIU/mL]
        C --> D[만삭까지 안정적 저평형 Plateau 유지]
    end
    subgraph "스테로이드 호르몬"
        E[임신 8주 이전: 난소 황체 전담] --> F[임신 8~12주: 황체-태반 전환기]
        F --> G[임신 후기: 태반 전담 및 만삭 직전까지 지속 상승]
    end
```
...
```mermaid
graph TD
    Z[수정란 Zygote] -->|수정 후 0~3일 분열: 상실배기| A[이융모막-이양막 DCDA: 25~30%]
    Z -->|수정 후 4~8일 분열: 포배기 내세포괴| B[일융모막-이양막 MCDA: 70~75%]
    Z -->|수정 후 9~12일 분열: 착상 후 배반| C[일융모막-일양막 MCMA: 1~2%]
    Z -->|수정 후 13일 이후 불완전 분열: 원시선| D[결합 쌍태아 Conjoined Twins: 극희귀]

    A --> A1[2개 태반, 2개 양막강, 람다 징후]
    B --> B1[1개 태반, 2개 양막강, T 징후, TTTS 위험]
    C --> C1[1개 태반, 1개 양막강, 탯줄 꼬임 극위험]
    D --> D1[샴쌍둥이, 높은 태내 사망]
```
...
sequenceDiagram
    autonumber
    participant E as 1. 진입 (Engagement)
    participant D as 2. 하강 (Descent)
    participant F as 3. 굴곡 (Flexion)
    participant IR as 4. 내회전 (Internal Rotation)
    participant Ex as 5. 신전 (Extension)
    participant ER as 6. 외회전 (External Rotation / Restitution)
    participant Exp as 7. 만출 (Expulsion)

    Note over E: 태아 아두의 대횡경선(BPD, 9.5cm)이 골반입구(Pelvic inlet) 평면을 통과하여 Station 0 도달
    Note over D: 자궁수축, 양수압 및 복부근육 압력으로 골반저를 향해 전 분만기 동안 지속 하강
    Note over F: 자궁경관 및 골반저 저항으로 턱이 가슴에 밀착, 후두하전두경선(OF, 11.5cm) 대신 소사경(SOB, 9.5cm) 제시
    Note over IR: 골반 출구의 전후경선에 맞추어 후두(Occiput)가 전방 치골결합 쪽으로 회전 (OA 위치 확립)
    Note over Ex: 골반저 만곡(Carus curve)을 따라 전방 상방으로 꺾이며 후두-전두-이마-코-턱 순으로 질구 신전 탈출
    Note over ER: 분만된 머리가 어깨의 축에 맞추어 원래 사위로 45도 복원(Restitution) 후, 어깨 회전과 함께 45도 외회전
    Note over Exp: 전방 어깨가 치골궁 밑으로 먼저 빠져나오고, 후방 어깨가 회음부를 통과한 후 몸통 급속 만출
```
...
```mermaid
graph TD
    Stage1[제1단계: 태반 형성 부전 Abnormal Placentation]
    ...
    E --> F[모체 전신 혈관내피세포 기능부전 Endothelial Dysfunction]
    F --> G1[혈관 투과성 증가: 전신 부종, 단백뇨, 뇌부종]
    F --> G2[전신 세동맥 연축: 중증 고혈압, 간/신장 허혈]
    F --> G3[응고계 활성화 및 미세혈전: 혈소판 감소, HELLP, DIC]
```
...
```mermaid
graph TD
    Endo[패혈성 자궁내막염 Septic Endometritis] --> Path1[1. 복막염 Peritonitis]
    Endo --> Path2[2. 자궁방조직염/골반봉와직염 Parametritis]
    Endo --> Path3[3. 골반혈전정맥염 Pelvic Thrombophlebitis]
    Endo --> Path4[4. 대퇴혈전정맥염 Femoral Thrombophlebitis]
    Endo --> Path5[5. 패혈성 전이 색전 Septic Pulmonary Infarct / Abscess]

    Path1 --> P1[난관을 통한 복강 파급 또는 자궁벽 관통 $\rightarrow$ 전반적 복막염, 복벽 강직, 장마비]
    Path2 --> P2[자궁 림프관을 통해 광인대 기저부 및 골반 후복막강 침윤 $\rightarrow$ 골반 연조직 봉와직염]
    Path3 --> P3[자궁정맥, 난소정맥, 내장골정맥 내 감염 혈전 $\rightarrow$ 패혈성 골반혈전정맥염 SPT]
    Path4 --> P4[장골정맥에서 대퇴정맥으로 혈전 진전 $\rightarrow$ 유백소구성 유통각 Phlegmasia alba dolens]
    Path5 --> P5[골반 정맥 혈전이 우심계를 거쳐 폐동맥 차단 $\rightarrow$ 패혈성 폐색전증, 폐농양, 간농양]
```
