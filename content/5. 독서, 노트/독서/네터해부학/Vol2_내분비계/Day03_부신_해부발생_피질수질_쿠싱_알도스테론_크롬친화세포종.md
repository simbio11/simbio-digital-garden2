...
```mermaid
graph TD
    A[임신 5-6주: 비뇨생식릉 체강 중배엽] -->|증식 및 침투| B[원시/태아 피질 Fetal Cortex]
...
```mermaid
graph TD
    A[쿠싱 의심 증상 환자] --> B[외인성 글루코코르티코이드 복용력 배제]
    B --> C[1단계: 고코르티솔혈증 선별 확진 검사 2가지 이상 시행]
    C -->|1 24시간 소변 유리 코르티솔 UFC| D{결과 판정}
    C -->|2 야간 1mg 덱사메타손 억제검사 DST| D
    C -->|3 야간 타액 코르티솔 측정| D
    D -->|정상| E[쿠싱 증후군 배제]
    ...
    M -->|MRI상 6mm 초과 선종 확인| N[쿠싱병 Cushing Disease 확진]
    M -->|모호하거나 6mm 이하| O[양측 하추체정맥동 채혈 IPSS 시행]
    O -->|중추/말초 ACTH 비: 기저 > 2, CRH 후 > 3| N
    O -->|비율 증가 없음| P[이소성 ACTH 증후군: 소세포폐암, 신경내분비종양 등]
```
...
sequenceDiagram
    ...
    Med->>P: 2단계: 고염식 섭취 및 다량 수액 공급 (수축된 혈관내 용적 재확장)
    Note over P,Med: 혈관 이완 후 반사성 빈맥 발생 시
    Med->>P: 3단계: 베타차단제(Propranolol 등) 후속 투여 (절대 단독 선행 금지!)
    Note over P,Surg: 혈압 안정, 기립성 활력징후 확인 후 수술실 입실
    Surg->>P: 4단계: 동맥관 거치 하에 종양 조작 최소화하며 복강경 적출
```
