...
```mermaid
graph TD
    A[혈중 포도당 농도 상승] --> B[GLUT2 / GLUT1 수송체 매개 촉진확산 유입]
    B --> C[글루코키나아제 Glucokinase / GCK 인산화: 속도결정 포도당 감지기]
    C --> D[해당과정 및 미토콘드리아 TCA 회로 / 산화적인산화]
    D --> E[세포질 ATP / ADP 비율 급격한 상승]
    E --> F[ATP-민감성 K+ 통로 K_ATP Channel 폐쇄]
    F --> G[세포막 K+ 유출 차단 -> 세포막 탈분극 -70mV to -30mV]
    G --> H[전압의존성 L형 Ca2+ 통로 개방]
    H --> I[세포 외 Ca2+ 대량 유입 -> 세포질 유리 Ca2+ 급증]
    I --> J[SNARE 복합체 활성화 -> 인슐린 분비과립 도킹 및 세포외유출 Exocytosis]
```
...
```mermaid
graph TD
    A[절대적 인슐린 결핍 + 스트레스 호르몬 급증] --> B[지방조직 HSL 활성화 -> 유리지방산 FFA 혈중 대량 방출]
    B --> C[간 유입: 인슐린 결핍으로 말로닐-CoA 격감]
    C --> D[CPT-1 탈억제 -> FFA 미토콘드리아 내막 진입 급증]
    D --> E[미토콘드리아 베타-산화 폭증 -> 아세틸-CoA 과부하]
    E --> F[TCA 회로 수용 한계 초과 -> HMG-CoA 합성 경로 가동]
    F --> G[케톤체 생성: 아세토아세테이트 & 베타-히드록시부티레이트]
    G --> H[혈중 H+ 축적 -> 고음이온간극 대사성 산화증 HAGMA]
    H --> I[호흡 보상: 쿠스마울 호흡 Kussmaul Breathing]
```
...
```mermaid
graph TD
    A[모체 고혈당증 Maternal Hyperglycemia] -->|촉진확산 GLUT1 통해 태반 자유 통과| B[태아 고혈당증 Fetal Hyperglycemia]
    C[모체 인슐린 Maternal Insulin] -->|태반 통과 절대 불가!| D[태반 장벽]
    B --> E[태아 췌장 랑게르한스섬 자극 -> β-세포 과형성]
    E --> F[태아 고인슐린혈증 Fetal Hyperinsulinemia]
    F -->|강력한 동화 성장호르몬 작용| G[거대아 Macrosomia: 간비대, 심비대, 복부지방 축적]
    F -->|출생 순간 탯줄 결찰 -> 포도당 공급 차단| H[신생아 중증 저혈당증 Neonatal Hypoglycemia]
    F -->|폐포 제2형 세포의 코르티솔 매개 계면활성제 합성 방해| I[신생아 호흡곤란증후군 RDS]
    F -->|태아 대사율 증가 및 저산소증 -> EPO 분비| J[적혈구증가증 Polycythemia & 신생아 황달]
```
