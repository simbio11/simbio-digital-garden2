...
```mermaid
graph TD
    subgraph 여포세포 내부 및 콜로이드 관강
        NIS[1. 요오드 포획 Trapping<br>기저측막 NIS 2Na+-I- 동반수송] --> Pendrin[2. 관강 수송 Efflux<br>정단막 펜드린 Pendrin 통과]
        Pendrin --> Oxid[3. 요오드 산화 Oxidation<br>TPO + H2O2 매개: I- -> I0]
        Oxid --> Organ[4. 유기화 Organification<br>TPO가 Tg 티로실 잔기에 요오드 결합 -> MIT / DIT]
        Organ --> Couple[5. 결합 반응 Coupling<br>DIT + DIT = T4<br>DIT + MIT = T3]
        Couple --> Storage[콜로이드 내 티로그로불린 Tg 형태로 저장]
        Storage --> Endocytosis[TSH 자극 -> 음세포작용 Endocytosis]
        Endocytosis --> Proteolysis[리소좀 단백분해 Proteolysis]
        Proteolysis --> Release[순환혈류 방출: T4 80% / T3 20%]
    end
```
...
```mermaid
graph LR
    subgraph 드퀘르벵 갑상선염 3단계 임상 경과
        Phase1[1기: 갑상선독증기 2~6주<br>여포 파괴로 저장된 T4/T3 대량 유출<br>RAIU 극저하 < 1%] --> Phase2[2기: 일시적 기능저하증 2~8주<br>저장량 고갈 및 합성 중단<br>TSH 상승, T4 저하]
        Phase2 --> Phase3[3기: 회복기 2~6개월<br>여포세포 재생 및 호르몬 완전 정상화<br>95% 이상 영구 결손 없이 완치]
    end
```
