...
```mermaid
graph TD
    subgraph Right_Kidney ["우신 전면 인접 장기 복합체"]
        RK_Sup["상극: 우측 부신 (Right Suprarenal Gland) - 비복막면"]
        RK_Lat["외측 상부: 간 우엽 (Hepatic Area) - 복막면, 간신와(Morison's Pouch)"]
        RK_Med["내측 연: 십이지장 하행부 (2nd Duodenum) - 비복막면"]
        RK_Inf["하극 외측: 결장우곡 / 간곡 (Right Colic Flexure) - 비복막면"]
        RK_Low["하극 내측: 소장 회장 고리 (Ileal Area) - 복막면"]
    end

    subgraph Left_Kidney ["좌신 전면 인접 장기 복합체"]
        LK_Sup["상극 내측: 좌측 부신 (Left Suprarenal Gland) - 비복막면"]
        LK_UpLat["외측 상부: 비장 (Splenic Area) - 복막면"]
        LK_Cent["중앙 횡대: 췌장 체미부 및 비장혈관 (Pancreatic Area) - 비복막면"]
        LK_UpMed["상부 복강: 위저부/체부 (Gastric Area) - 복막면, 망낭 바닥"]
        LK_InfLat["하극 외측: 결장좌곡 / 비곡 및 하행결장 (Left Colic Area) - 비복막면"]
        LK_InfMed["하극 내측: 소장 공장 고리 (Jejunal Area) - 복막면"]
    end
```
...
```mermaid
graph LR
    Parenchyma["신장 실질 (Parenchyma)"] --> Layer1["1. 진성 섬유피막 (Fibrous Capsule)"]
    Layer1 --> Layer2["2. 신주위지방 (Perinephric / Perirenal Fat)"]
    Layer2 --> Layer3["3. 신근막 (Renal Fascia)<br>- 전방: 게로타 (Gerota)<br>- 후방: 주커칸들 (Zuckerkandl)"]
    Layer3 --> Layer4["4. 신방지방 (Paranephric / Pararenal Fat)"]
    Layer4 --> BodyWall["복벽 가로근막 (Transversalis Fascia)"]
```
...
```mermaid
graph TD
    subgraph Male_Peritoneum ["남성 골반 복막 반전선"]
        M1["방광 상면 복막"] --> M2["직장방광와 (Rectovesical pouch)<br>남성 복막강 최하단"]
        M2 --> M3["직장 전면"]
    end

    subgraph Female_Peritoneum ["여성 골반 복막 반전선"]
        F1["방광 상면 복막"] --> F2["방광자궁와 (Vesicouterine pouch)"]
        F2 --> F3["자궁 전면 $\rightarrow$ 저부 $\rightarrow$ 후면"]
        F3 --> F4["직장자궁와 (더글라스강, Pouch of Douglas)<br>여성 복막강 최하단"]
        F4 --> F5["직장 전면"]
    end
```
...
```mermaid
graph TD
    MRA["주 신동맥 (Main Renal Artery)"] --> AntDiv["전지 (Anterior Division)"]
    MRA --> PostDiv["후지 (Posterior Division)"]

    AntDiv --> Seg1["1. 정점 분절동맥 (Apical / Superior Segmental a.)"]
    AntDiv --> Seg2["2. 전상 분절동맥 (Anterosuperior Segmental a.)"]
    AntDiv --> Seg3["3. 전하 분절동맥 (Anteroinferior Segmental a.)"]
    AntDiv --> Seg4["4. 하 분절동맥 (Inferior Segmental a.)"]

    PostDiv --> Seg5["5. 후 분절동맥 (Posterior Segmental a.)"]
```
...
```mermaid
graph TD
    SMA["상장간막동맥 (SMA)"] -->|전방 압박| LRV["좌측 신정맥 (Left Renal Vein)"]
    Aorta["복부대동맥 (Abdominal Aorta)"] -->|후방 지지대| LRV
    LRV --> Stasis["신정맥 내압 상승 및 만성 울혈"]
    Stasis --> FlankPain["좌측 측복통 (Flank pain)"]
    Stasis --> Hematuria["신배 정맥류 파열 $\rightarrow$ 미세/육안적 혈뇨"]
...
```mermaid
graph TD
    subgraph Storage ["1. 소변 저장기 (Filling/Storage Phase)"]
        Symp["교감신경 활성화 (T11-L2, 하복신경)<br>① 방광체부 β3 수용체: 배뇨근 이완<br>② 방광경부 α1 수용체: 내요도괄약근 수축"]
        Somatic1["체성신경 톤 유지 (S2-S4, 음부신경)<br>외요도괄약근(EUS) 니코틴 수용체 지속 수축"]
    end

    subgraph Voiding ["2. 배뇨기 (Voiding/Micturition Phase)"]
        Parasymp["부교감신경 활성화 (S2-S4, 골반내장신경)<br>방광 배뇨근 M2/M3 수용체 자극: 강력한 수축"]
        Inhibit["교감신경 억제<br>방광경부/내괄약근 이완"]
        Somatic2["체성신경 수의적 억제<br>외요도괄약근 완전 이완"]
    end
```
...
| :--- | :--- | :--- | :--- | :--- |
| **교감신경 (Sympathetic)** | **T11 ~ L2** IML 측각 $\rightarrow$ 하복신경(Hypogastric n.) | **이완** ($\beta_3$ 아드레날린 수용체) | **수축** ($\alpha_1$ 아드레날린 수용체, 내괄약근 폐쇄) | Mirabegron ($\beta_3$ 효능제: 과민성방광 치료)<br>Tamsulosin ($\alpha_1$ 차단제: 전립선비대 배뇨장애 개선) |
| **부교감신경 (Parasympathetic)** | **S2 ~ S4** 천골 부교감핵 $\rightarrow$ 골반내장신경(Pelvic splanchnic n.) | **강력한 수축** ($M_2, M_3$ 무스카린 수용체) | 간접적 이완 유도 | Solifenacin ($M_3$ 차단제: 절박뇨/빈뇨 억제)<br>Bethanechol (무스카린 효능제: 신경인성 방광 배뇨 촉진) |
| **체성신경 (Somatic)** | **S2 ~ S4** 천수 전각 Onuf 핵 $\rightarrow$ 음부신경(Pudendal n.) | 작용 없음 | **외요도괄약근(EUS) 수의적 수축** ($N_M$ 니코틴 수용체) | 케겔 운동 (골반저근 강화)<br>근이완제 |
...
| **헨레고리 길이** | **단순 단축 루프 (Short loop)**<br>외수질 경계까지만 진입 후 반전 | **장축 심부 루프 (Long loop)**<br>내수질 및 신유두 끝까지 깊숙이 침투 |
...
```mermaid
graph LR
    Afferent["수입소동맥 (Afferent arteriole)<br>고압 저항혈관"] --> GlomCap["사구체 모세혈관망<br>높은 정수압 (~50 mmHg)<br>여과(Filtration) 특화"]
    GlomCap --> Efferent["수출소동맥 (Efferent arteriole)<br>가변 저항혈관"]
    Efferent --> SecCap["2차 모세혈관상<br>- 세뇨관주위 모세혈관망<br>- 직세관 (Vasa recta)<br>낮은 정수압 (~10 mmHg) + 높은 교질삼투압 (~35 mmHg)<br>재흡수(Reabsorption) 특화"]
    SecCap --> Vein["신정맥 계통 (Venous return)"]
```
...
```mermaid
graph TD
    subgraph GFB ["사구체 여과장벽 (Glomerular Filtration Barrier)"]
        Endo["1. 유창 내피세포 (Fenestrated Endothelium)<br>- 세공 크기: 70~100 nm (혈구 통과 차단)<br>- 음전하 글리코칼릭스 코팅"]
        GBM["2. 사구체 기저막 (GBM, 300~350 nm)<br>- Lamina rara interna (음전하)<br>- Lamina densa (4형 콜라겐 α3α4α5 그물망)<br>- Lamina rara externa (음전하 Agrin/헤파란황산)"]
        Podo["3. 족세포 발돌기 및 슬릿막 (Slit Diaphragm)<br>- 발돌기 간격: 25~60 nm<br>- 슬릿 여과 세공: 4 x 14 nm<br>- 네프린(Nephrin), 포도신(Podocin), CD2AP 복합체"]
    end
```
...
```mermaid
graph TD
    JGA["사구체옆장치 (Juxtaglomerular Apparatus)"] --> Macula["1. 치밀반 (Macula Densa)<br>- 원위직세뇨관/TAL 전이부 특수 상피세포<br>- 관강 내 NaCl 농도 감지 (NKCC2)<br>- 세뇨관사구체되먹임(TGF) 매개"]
    JGA --> JG_Cells["2. 사구체옆세포 (JG Cells / Granular Cells)<br>- 수입소동맥 중막의 변형된 평활근세포<br>- 신동맥 압력수용체 (Baroreceptor)<br>- 레닌(Renin) 합성·저장·분비"]
...
| :--- | :--- | :--- | :--- | :--- |
| **하행 세근 (DTL)** | **극도로 높음**<br>(정단·기저측막 **AQP1** 대량 발현) | **불투과 (Impermeable)**<br>(능동수송 및 수동통로 없음) | 매우 낮음 | 수질 간질의 고삼투압에 의해 물이 빠져나가 관강액이 **고장액($1,200\text{ mOsm/kg}$)으로 농축**됨 |
| **상행 세근 (ATL)** | **완전 불투과 (Impermeable)**<br>(AQP1 전무, 밀착연접 치밀) | **수동 투과성 매우 높음**<br>(ClC-K1 염소 통로 발현) | 수동 투과성 높음 (UT-A2) | 물 이동 없이 $\text{NaCl}$만 간질로 수동 확산되어 관강액이 **희석**되기 시작함 |
...
```mermaid
graph TD
    CCD["피질 집합관 상피세포"] --> Principal["1. 주세포 (Principal Cell, ~70%)<br>- ENaC: Na+ 재흡수 (알도스테론 촉진)<br>- ROMK: K+ 분비 (고칼륨혈증 방지)<br>- AQP2: 바소프레신(ADH) 의존적 수분 재흡수"]
    CCD --> Intercalated["2. 개재세포 (Intercalated Cell, ~30%)<br>- 미토콘드리아와 탄산탈수효소(CA-II) 풍부<br>- 산-염기 평형 정밀 조절"]
    Intercalated --> TypeA["Type A (α-개재세포)<br>정단 H+-ATPase (산 분비)<br>기저측 AE1 (HCO3- 혈중 회수)<br>대사성 산증 시 활성화"]
    Intercalated --> TypeB["Type B (β-개재세포)<br>정단 Pendrin (HCO3- 요중 분비)<br>기저측 H+-ATPase<br>대사성 알칼리증 시 활성화"]
```
