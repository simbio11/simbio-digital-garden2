...
- 성인의 체중 감소 및 소아의 성장 부진을 주요 임상 특징으로 규정하며, **[[부종]] 체액 저류(fluid retention)를 보정한 순수 체중 감소**를 진단에 적용.
...
| **선택 (1개 이상)** | **가) 주관적 증상**: 식욕 부진(Anorexia)<br>**나) 객관적 기능 측정**: 악력(Handgrip strength) 저하 (**남성 < 28 kg, 여성 < 18 kg**)<br>**다) 전신 염증 바이오마커**: CRP 상승 (**> 5 mg/L** 또는 **> 0.5 mg/dL**) |
...
> ![[Sarcopenia_Ch18_p295_Fig1.png|540]]
...
```mermaid
flowchart LR
    A["전악액질 (Precachexia)<br>• 체중 감소 ≤ 5%<br>• 식욕부진 & 대사변화 (포도당 불내성 등)"] --> B["악액질 (Cachexia)<br>• 체중 감소 > 5% (6개월 내)<br>• 또는 BMI < 20 & 체중감소 > 2%<br>• 또는 근감소증 & 체중감소 > 2%<br>• 식이 섭취 감소 및 전신염증"]
    B --> C["난치성 악액질 (Refractory Cachexia)<br>• 암 치료 불응 & 급속 진행<br>• 신체 수행능력 저하 (ECOG 3~4)<br>• 기대 여명 < 3개월<br>• 완화의료 중심"]
```
...
- 체중 감소 환자 중 골격근이 소모되어 [[근감소증]] 동반된 경우가 체단백질이 유지된 경우보다 예후가 현저히 불량하다.
...
```mermaid
flowchart TD
    subgraph Etiology ["원인 및 매개 인자"]
        T["악성 종양 / 만성 질환<br>(암, 심부전, COPD, 신장병)"]
        CK["전신 염증 사이토카인 분비<br>(TNF-α, IL-6, IL-1, IFN-γ)"]
        T --> CK
    end

    subgraph Pathology ["병태생리적 교란"]
        CNS["시상하부 중추 교란<br>• MC4R 활성화, NPY 억제<br>• 중추성 식욕부진 (Anorexia)"]
        HYP["대사율 항진 (Hypermetabolism)<br>• 음성 에너지 균형<br>• 체지방 소모"]
        MUS["단백질 분해 가속화<br>• 유비퀴틴-프로테아좀 경로<br>• 음성 질소 균형 → 골격근 소실"]
        CK --> CNS
        CK --> HYP
        CK --> MUS
    end

    subgraph Clinical ["임상 결과"]
        CAC["악액질 (Cachexia)<br>• 불응성 체중 감소<br>• 근감소증 (Sarcopenia)<br>• 치료 독성 증가 & 사망률 상승"]
        CNS --> CAC
        HYP --> CAC
        MUS --> CAC
    end

    subgraph Treatment ["다차원적 치료 전략"]
        DRUG["약물 요법<br>• 올란자핀: 식욕 개선 & 체중 증가<br>• MEGACE: NPY 자극 & 염증 억제<br>• 아나모렐린: 그렐린 수용체 작용<br>• 탈리도마이드 / NSAIDs / EPA"]
        NUT["영양 요법<br>• 에너지 +300~400 kcal/일<br>• 고단백 1.0~1.5 g/kg/일"]
        EX["운동 요법<br>• 저항성 운동 (근육량 보존)<br>• 유산소 운동 (피로 완화)"]
    end
