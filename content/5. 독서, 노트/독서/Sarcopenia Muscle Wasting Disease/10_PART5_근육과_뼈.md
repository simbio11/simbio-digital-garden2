...
```mermaid
flowchart LR
    A["사지 근육량 (ALM)<br>및 대퇴사두근력"] -->|양의 상관관계| B["대퇴골 및 척추<br>골밀도 (BMD)"]
    C["근육량 지수 저하<br>(근감소증)"] -->|유의한 위험 연관| D["골다공증 및 골절<br>(Bone Fragility)"]
    A -->|기하학적 적응| E["골 구조 안정성<br>(Bone Geometry)"]
```
...
1. **고관절 골절 환자 대상 연구 (Di Monaco 등, 313명)**
   ...
   - 근육량 지수로 정의한 [[근감소증]] 유무는 [[골다공증]] 존재와 유의한 상관성을 나타냄.
2. **일본 대규모 코호트 연구 (Miyakoshi 등, 2,400여 명)**
   - 40~88세 폐경 전후 여성을 대상으로 조사한 결과, 골감소증 및 [[골다공증]] 환자군에서 [[근감소증]] 동반 비율이 전 연령대에 걸쳐 유의하게 높음.
...
```mermaid
flowchart TD
    subgraph S1 ["1. Mechanostat Theory"]
        M1["근육 수축 및 운동 부하"] -->|기계적 변형 자극| B1["뼈 리모델링 자극 & 골강도 증가"]
        M2["비사용 / 신체활동 감소"] -->|기계적 무하중| B2["골밀도 급감 & 골절 위험 증가"]
    end
    subgraph S2 ["2. Common Aging Process"]
        A1["성장호르몬 / 성호르몬 감소"] --> BM1["근육량 & 골밀도 동시 감소"]
        A2["TNF-α, IL-6 등 염증성 사이토카인 증가"] --> BM2["이화작용 촉진 (근위축 + 골흡수)"]
        A3["산화 스트레스 증가 & 비타민 D 결핍"] --> BM3["골·근육 동시 악화"]
    end
    subgraph S3 ["3. Muscle to Bone (Myokines)"]
        MK["근육 분비 마이오카인"] -->|Irisin, IGF-1, FGF-2, IL-15| B3["조골세포 활성화 / 피질골 강화"]
        MK2["Myostatin"] -->|이화 작용| B4["조골세포 억제"]
    end
    subgraph S4 ["4. Bone to Muscle (Osteokines)"]
        OK1["Osteocalcin"] -->|동화 작용| M3["근육량 유지 & 대사 분화 촉진"]
        OK2["Sclerostin"] -->|이화 작용| M4["근모세포 분화·증식 억제"]
    end
```
...
- **만성 전신 염증(Inflammaging)**: TNF-$\alpha$, [[IL-6]] 등 전염증성 [[사이토카인]] 증가와 활성산소종([[ROS]])에 의한 산화 스트레스 축적은 근단백질 분해와 파골세포 활성화를 동시에 촉진.
- **[[비타민 D]](Vitamin D) 결핍**: [[칼슘]] 흡수 저하뿐 아니라 근세포 내 VDR([[비타민 D]] 수용체) 신호 전달을 약화시켜 근력 약화와 골연화/[[골다공증]] 동반 유발.
...
- **[[골근감소증]](Osteosarcopenia) 개념의 태동**:
  - 2009년 Binkley & Buehring이 [[골다공증]] [[근감소증]] 개념을 통합한 `sarco-osteopenia` 용어를 처음 제안.
...
- **유전 가능도(Heritability)**:
  ...
  - 폐경 후 여성의 근력: 약 30~50%.
  - [[대퇴골]] 경부 골강도: 약 40~50%.
- **공유 유전 성분**: 10,414명의 소아 대상 메타분석 결과, [[제지방량]] 골밀도 간의 공유 유전 성분은 43%, SNP 기반 유전 가능도는 39%로 추정됨.
...
- 한국인 4년 추적 연구: [[근감소증]] 향후 [[골다공증]] 발생을 유의하게 예측하였으며, [[골다공증]] 역시 근감소증의 위험을 상승시키는 양방향 예측 인자로 작용함.
...
| :--- | :--- | :---: | :--- |
| **근육 분비**<br>(Myokines) | **미오스타틴 (Myostatin)** | **이화 작용 (Catabolic)** | 조골세포(osteoblast) 기능 억제 및 골형성 저해; 억제 시 근육량 및 골강도 동반 증가 |
...
| **뼈 분비**<br>(Osteokines) | **오스테오칼신 (Osteocalcin)** | **동화 작용 (Anabolic)** | 조골세포 유래; 근섬유 동화 촉진, 하지 근력 유지, 췌장 $\beta$세포 증식 및 [[인슐린]] 분비 촉진 |
| | **스클레로스틴 (Sclerostin)** | **이화 작용 (Catabolic)** | 골세포 유래 당단백질; Wnt 신호 차단, 근모세포(myoblast) 증식 및 분화 억제 |

---

### 2.4 Osteosarcopenia의 역학, 유병률 및 예후
진단 기준([[골다공증]] WHO T-score $\le -2.5$로 확립되었으나, [[근감소증]] EWGSOP vs AWGS 등 연구마다 상이)에 따라 보고된 유병률에 차이가 존재함.
...
```mermaid
flowchart TD
    subgraph Multi ["낙상의 4대 복합 위험인자 모델"]
        Bio["1. 생물학적 / 의학적 인자<br>• 80세 이상 고령, 여성 우세<br>• 하지 근력 약화, 균형감각·협동기능 소실<br>• 파킨슨병, 뇌졸중 편마비<br>• 시력/대비감 저하, 암·명순응 장애<br>• 골관절염, 골다공증, 인지장애"]
        Beh["2. 행동학적 인자<br>• 낙상 과거력 및 낙상 공포증<br>• 위험 행동 반복<br>• 항우울제/진정제 등 약물 복용<br>• 과도한 음주, 부적절한 신발/의복"]
        Env["3. 환경적 인자<br>• 작은 깔개, 미끄러운 마루/욕실 바닥<br>• 정리되지 않은 전선, 문턱<br>• 계단 난간 부재, 야간 조명 부족<br>• 실외 빙판길"]
        Soc["4. 사회경제적 인자<br>• 낮은 사회경제적 수준, 독거<br>• 80세 이상 기혼 상태는 방어 효과"]
    end
    Bio & Beh & Env & Soc --> Risk["복합 상호작용에 의한 낙상 발생"]
```
...
```mermaid
flowchart LR
    Sarcopenia["근감소증<br>(근육량 감소 + 근력 저하 + 보행속도 감소)"]
    Sarcopenia --> Weak["하지 근력 약화<br>& 균형 감각 상실"]
    Weak --> Fall["낙상 위험 2.5~3.0배 증가"]
    Fall --> Fracture["골다공증성 취약 골절"]
    Fracture --> Death["침상 안정 & 사망률 증가"]
```
...
| **3. 낮은 [[제지방량]] (Low lean mass)** | DXA 측정 사지 [[제지방량]]($\text{ALM}/\text{height}^2$):<br>• **여성: $< 5.45\ \text{kg/m}^2$**<br>• **남성: $< 7.26\ \text{kg/m}^2$** | 양적 [[근감소증]] 기준 충족 |
| **4. 느린 보행속도 (Slow gait speed)** | 편안한 일상 보행 속도 **$< 1.0\ \text{m/s}$** | 이동성 및 근기능 저하 |
| **5. 낮은 악력 (Low grip strength)** | 휴대용 악력계(Hand-held dynamometer) 측정치:<br>• **남성: $< 30\ \text{kg}$**<br>• **여성: $< 20\ \text{kg}$** | 전신 근력 위약 지표 |
| **6. [[비만]] / 높은 지방량 (Obesity / High fat mass)** | DXA 측정 총 체지방률(Total body % fat):<br>• **남성: $> 30\%$**<br>• **여성: $> 40\%$** | 근육 내 지방 침윤 및 근감소성 비만 |
...
```mermaid
flowchart TD
    subgraph KURE ["한국형 Dysmobility Syndrome 평가 도구 (KURE 연구)"]
        A["골다공증 (T $\le$ -2.5)"]
        B["전년도 낙상력 ($\ge$ 1회)"]
        C["낮은 골격근량 (BIA ASM/ht²)<br>• 남성 < 7.9 kg/m²<br>• 여성 < 5.7 kg/m²"]
        D["신체 수행능력 저하 대체 평가<br>• TUG (Timed Up and Go) > 12초"]
        E["저악력 (남 < 30kg, 여 < 20kg)"]
        F["높은 체지방률 (남 > 30%, 여 > 40%)"]
    end
