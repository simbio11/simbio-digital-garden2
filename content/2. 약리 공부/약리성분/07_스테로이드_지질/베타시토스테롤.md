...
| **식물 내 생합성 경로** | 메발론산(MVA) 및 비메발론산(MEP) 경로를 통해 스쿠알렌(Squalene) $\rightarrow$ 시클로아르테놀 $\rightarrow$ β-시토스테롤 생합성 |
| **산업적 합성 전구체** | 아나볼릭 스테로이드인 볼데논(Boldenone) 및 안드로스테네디온 반합성을 위한 미생물 생전환(Biotransformation by *Mycobacterium*) 주요 출발 기질 |

### 1.2 2D 분자 구조식
![[베타시토스테롤_화학구조식.png|380]]
> [그림 1: ① 2차원 분자 화학구조식 — 베타시토스테롤(β-Sitosterol, PubChem CID: 222284). 콜레스테롤 골격의 24번 탄소에 에틸기가 치환된 식물성 스테롤 구조. 출처: PubChem]
...
![[베타시토스테롤_기원식물_황백.jpg|400]]
> [그림 2: ② 기원 식물 실사 — 베타시토스테롤을 풍부하게 함유하는 대표적 청열조습(淸熱燥濕) 본초인 황벽나무(황백, Phellodendron amurense). 출처: Wikimedia Commons (CC BY-SA 3.0)]
...
> [그림 3: ③ 간내 콜레스테롤 대사 경로 및 지질 조절 도해 — β-시토스테롤의 소장 미셀 내 콜레스테롤 경쟁적 배제 및 체내 흡수 차단 기전 — 출처: Wikimedia Commons (CC BY-SA 4.0)]
...
![[전립선_비대_및_염증_해부단면도.jpg|450]]
> [그림 4: ④ 전립선 비대 및 요도 압박 병태 해부도 — β-시토스테롤의 전립선 평활근 수축 억제 및 하부요로증상(LUTS) 개선 타겟 — 출처: Wikimedia Commons (CC BY-SA 4.0)]
