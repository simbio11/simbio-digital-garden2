...
![[아네모닌_화학구조식.png|320]]
> [그림 1: 아네모닌 2D 화학 분자 구조식 (출처: 위키미디어 공용 · NCI CACTVS 구조 데이터)]
...
* **간 대사 및 소실 반감기**: 데이터 없음(정량적 반감기·생체이용률 데이터 확인되지 않음).
...
<font color="#fa7e7e">⚠️ 내용 검토 필요: 위 연구는 모두 동물·세포 모델이며, 인체 대상 임상시험(RCT) 근거는 이번 검색 범위 내에서 확인되지 않았습니다.</font>

### 4) 항균·항진균 작용 (전구체 프로토아네모닌 관련 근거 — 아네모닌 자체 데이터와 구분 필요)
* **아네모닌 자체**에 대해 [[백두옹]]의 전통적 주치인 세균성·아메바성 이질(Shigella·Entamoeba 등)에 특이적인 항균 기전을 직접 검증한 현대 문헌은 이번 검색에서 확인하지 못했습니다(데이터 없음).
* 다만 아네모닌의 전구체인 **프로토아네모닌(Protoanemonin)** 자체는 고전 항진균 연구에서 시험관 내 항진균 활성이 보고된 바 있습니다(PMID 2356244, 1990년 *Planta Medica*). 이는 아네모닌이 아닌 전구체 물질에 대한 결과이므로, [[백두옹]]의 청열해독·이질 치료 효능을 아네모닌 항균 기전으로 직접 연결하는 것은 근거가 불충분하며 과잉 해석하지 않도록 유의가 필요합니다.
...
3. **Martín ML, et al. (1990)** — "In vitro activity of protoanemonin, an antifungal agent". *Planta Medica*, 56(1), 66–69. PMID: [2356244](https://pubmed.ncbi.nlm.nih.gov/2356244/).
   - **[연구 요약]**: 아네모닌의 전구체인 프로토아네모닌의 시험관 내 항진균 활성을 규명 — **아네모닌 자체가 아닌 전구체 물질**에 대한 결과임에 유의.
4. **PubChem CID 10496 (Anemonin)**; **CAS 508-44-1**. National Center for Biotechnology Information.
   - **[화학 데이터베이스]**: 분자식·CAS 번호 등 공인 화학 데이터.

> ⚠️ 내용 검토 필요: §2의 백두옹 내 지표 함량(%) 수치는 이번에도 확인된 문헌이 없어 데이터 없음으로 유지합니다. §5④에서 밝혔듯 [[백두옹]] 전통 주치(이질)에 대한 아네모닌 **자체**의 특이적 항균·항아메바 기전 문헌은 확인되지 않았으며, 전구체 프로토아네모닌의 항진균 문헌(PMID 2356244)만 확인되어 혼동하지 않도록 구분했습니다. §4의 정량적 ADME 데이터, §7의 CYP450 상호작용 데이터도 확인된 문헌이 없습니다.

<!-- 보관 태그(1회용·링크오류, 필요시 복원): 락톤, 백두옹 -->
