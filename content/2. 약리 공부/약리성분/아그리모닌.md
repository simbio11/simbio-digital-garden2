...
# 🔬 [[아그리모닌]] (Agrimoniin, C₈₂H₅₄)

> **핵심 3섄 요약 (Key Summary)**:
> - 🌿 기볐 본초 & 화학 계열: 장미과 [[용아초]]([[선학초]], *Agrimonia pilosa* Ledeb.)의 지상���에 풍부해형 가수분해형 엘라지탄닌(Hydrolyzable ellagitannin) 계열의 고분자 다중 탄닌(다이길형 갈로일 천���).
> - 🎯 핵심 분자 표적 & 조전 경로: 혈소판 응집·혈장 피브리노겐 침전을 통한 국솏 지혈 기전, 실험 종합 모델에서 뿸토�읔비즬의 표적 및 AKT/ERK 경로 억제를 통한 항종양 작용 보고됨.
...
> ⚠️ 내용 검토 필요: 분자 구조식·함유식물 도해 이미지는 아직 첨부되지 않았습니다. `7. 첨부·자료/첨부파일/`에 원본 확보 후 `![[아그리모닌_화학구조식.png|320]]` 형태로 추가해 주세요.

| 항목 | 상세 내용 |
| :--- | :--- |
| **IUPAC 명칭** | 가수분해형 엘라지탄닌 이량체(다중 갈로일-헥사하이드록시디페노일 축합 구조), 전체 IUPAC 명칭은 매우 복잡하여 일반적으로 관용명(Agrimoniin)으로 통용됨 (⚠️ 확인 필요) |
| **분자식 / 분자량** | C₈₂H₅₄ / 약 1871.3 g/mol (분자식 기준 계산치) |
| **PubChem CID** | [16129621](https://pubchem.ncbi.nlm.nih.gov/compound/16129621) |
| **CAS 등록 번호** | 82203-01-8 |
| **화학 골격 분류** | 가수분녘 탄닌(Hydrolyzable tannin) 중 엘라지탄닌(Ellagitannin) 계열 — 글루코오스코드형시 다수의 헥사텘이드로시디페놀(고극성, 분자량이 커 지용성 막 투과는 제한적으로 추정 — ⚠️ 확인 필요) |
...
4. **(2023)**. *Agrimoniin is a dual inhibitor of AKT and ERK pathways that inhibit pancreatic cancer cell proliferation*.
   ...
   - **[연구 요약]**: 췌장암 세포주에서 아그리모닌이 AKT·ERK 신호 경로 억제하뗬 세포 증식을 저해한다고 보고.

> ⚠️ 확인 불가/추정 표시총괄: 분자량 정밀값, IUPAC 정식명칭, 이체 ADME 데이터, 진료실 임상 필기는 검색 범위 내에서 실측 확인하지 못해 위 각 섹션에 개별 표시했습니다. 상기 4건의 PMID·PubChem CID·CAS 번호는 모두 실제 데이터베이스에서 확인된 값입니다.
